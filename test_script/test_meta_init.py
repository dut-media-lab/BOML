...
def build(metasets, learn_lr, lr0, MBS, T, mlr0,mlr_decay=1.e-5, process_fn=None, method='MetaInit',inner_method='Simple', outer_method='Simple',
          use_T=False, use_Warp=False, first_order=False, warp_lambda=1.0):
    ...
    for k, ex in enumerate(exs):
        ...
        boml_ho.ul_problem(outer_objective=ex.errors['validation'], meta_learning_rate=mlr0, inner_grad=optim_dict,
                            outer_objective_optimizer=args.outer_opt,mlr_decay=mlr_decay, warp_lambda=warp_lambda,
...
def train_and_test(metasets, name_of_exp,method, inner_method, outer_method, use_T=False, use_Warp=False, warp_lambda=1.0,
    ...
    exs, boml_ho, saver = build(metasets, learn_lr, lr0,MBS, T, mlr0,mlr_decay, process_fn,
                                 method, inner_method, outer_method, use_T, use_Warp, first_order,warp_lambda)
...
# training and testing function
def build_and_test(metasets, exp_dir,method, inner_method, outer_method, use_T=False,use_Warp=False,warp_lambda=1.0, first_order=False,
    ...
    exs, pybml_ho, saver = build(metasets, learn_lr, lr0, MBS, T, mlr0,
                                 mlr_decay,process_fn,method, inner_method, outer_method, use_T,use_Warp, first_order,warp_lambda)
...
def test_meta_init():
    ...
    if args.mode == 'train':
        train_and_test(metasets, exp_string,method=args.method, inner_method=args.inner_method,
                       ...
                       seed=args.seed,use_Warp=args.use_Warp, warp_lambda=args.warp_lambda,
