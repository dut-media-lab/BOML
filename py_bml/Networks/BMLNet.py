...
from py_bml.Networks.network_utils import filter_vars
from py_bml.extension import GraphKeys
from py_bml.utils import remove_from_collection, as_tuple_or_list
...
class BMLNet(object):
    ...
    def create_outer_parameters(self, var_collections=GraphKeys.METAPARAMETERS):
        '''
        this method creates parameters of upper level problems, and adds
        them to define collections called METAPARAMETERS
        :param var_collections: collections to restore meta parameters created in the so called scope
        :return: dictionary that indexes the outer parameters
        '''
...
class BMLNetFeedForward(BMLNet):
    def __init__(self, _input, dims, task_parameter=None,name='BMLNetFeedForward', activation=tf.nn.relu,
        ...
        self.use_T = use_T
        super().__init__(_input=_input, name=name, var_collections=var_collections,
    ...
    def create_initial_parameter(self):
        ...
        self.task_parameter['fc_weight'] = tf.get_variable('fc_weight',
                                                                                 [self.layers[-1].shape.as_list()[-1],self.dims[-1]],
                                                                                 initializer=self.output_weight_initializer,dtype=self.data_type)
    ...
    def re_forward(self, new_input=None, task_parameter=OrderedDict()):
        return BMLNetFeedForward(new_input if new_input is not None else self.layers[0], dims=self.dims,
                                 task_parameter=self.task_parameter if len(task_parameter.keys()) ==0 else task_parameter ,
                                 name=self.name, activation=self.activation,data_type=self.data_type,
