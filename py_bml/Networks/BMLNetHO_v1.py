...
from py_bml.Networks.network_utils import as_tuple_or_list
from py_bml.extension import GraphKeys
from py_bml.utils import remove_from_collection


class BMLNetHO_v1(BMLNet):
    ...
    def re_forward(self, new_input=None, task_parameter=OrderedDict()):
        return BMLNetHO_v1(_input=new_input if new_input is not None else self.layers[0],
...
def BMLNetOmniglotHO_v1(_input, dim_output, outer_param_dict=OrderedDict(), model_param_dict=OrderedDict(),
                        batch_norm=layers.batch_norm, name='BMLNetC4LOmniglot', use_T=False, use_Warp=False,**model_args):
    return BMLNetHO_v1(_input=_input, name=name, dim_output=dim_output,model_param_dict=model_param_dict,
                       outer_param_dict=outer_param_dict, norm=batch_norm, use_T=use_T, use_Warp=use_Warp,**model_args)


def BMLNetMiniHO_v1(_input, dim_output, outer_param_dict=OrderedDict(),model_param_dict =OrderedDict(),
                    batch_norm=layers.batch_norm, name='BMLNetC4LMini', use_T=False, use_Warp=False,**model_args):
    return BMLNetHO_v1(_input=_input, name=name, dim_output=dim_output, use_T=use_T, use_Warp=use_Warp,
