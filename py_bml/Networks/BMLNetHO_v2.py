...
from py_bml.Networks.BMLNet import *
from py_bml.extension import remove_from_collection


class BMLNetMiniHO_v2(BMLNet):
    ...
    def re_forward(self, new_input=None, task_parameter=OrderedDict()):
        return BMLNetMiniHO_v2(_input=new_input if (new_input is not None) else self.layers[0],
...
class BMLNetOmniglotHO_v2(BMLNet):
    ...
    def re_forward(self, new_input=None, task_parameter=None):
        return BMLNetOmniglotHO_v2(_input=new_input if new_input is not None else self.layers[0],
                                   dim_output=self.dims[-1], name=self.name, activation=self.activation,
                                   outer_param_dict=self.outer_param_dict,model_param_dict=self.model_param_dict,
