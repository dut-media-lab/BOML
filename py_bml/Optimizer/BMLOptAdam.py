...
class BMLOptAdam(BMLOpt, tf.train.AdamOptimizer):
    ...
    def __init__(self, learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-5, use_locking=False, name="Adam"):
        super(BMLOptAdam, self).__init__(learning_rate, beta1, beta2, epsilon, use_locking, name)

    def apply_gradients(self, grads_and_vars, global_step=None, name=None):
        update_op = super(BMLOptAdam, self).apply_gradients(grads_and_vars, global_step, name)
    ...
    def optimizer_params_tensor(self):
        return super(BMLOptAdam, self).optimizer_params_tensor + [self._beta1_t, self._beta2_t]
