...
from py_bml.LLProblem import BMLInnerGradTrad
from py_bml.utils import get_regularization, add_list


class BMLInnerGradSimple(BMLInnerGradTrad):
    ...
    def compute_gradients(bml_opt, loss_inner, loss_outer=None, param_dict=OrderedDict(),
        ...
        if len(param_dict) > 0 and len(var_list) > 0:
            fast_param, outer_grad, model_grad = BMLInnerGradSimple.bml_inner_grad_trad(
                                                                                                           loss_inner=loss_inner,param_dict=param_dict, var_list=var_list)

            return BMLInnerGradSimple(update_op=update_op, dynamics=dynamics, objective=loss_inner,
    ...
    def bml_inner_grad_trad(loss_inner, param_dict=OrderedDict(), var_list=[]):
        ...
        if param_dict['use_Warp']:
            outer_param_loss =param_dict['outer_loss_func'](pred=task_model.re_forward(param_dict['experiment'].x_).out,
                                                      label=param_dict['experiment'].y_, method='HyperOptim')
        ...
        for _ in range(param_dict['T'] - 1):
            ...
            iter_loss = param_dict['loss_func'](pred=task_model.out, label=param_dict['experiment'].y,method='HyperOptim')
            ...
            if 'regularization' in param_dict.keys():
                iter_loss = iter_loss + get_regularization(task_model.var_list,
                                                           regularization=param_dict['regularization'],
                                                           scalor=param_dict['scalor'])
    ...
    def __lt__(self, other):  # make OptimizerDict sortable
        ...
        assert isinstance(other, BMLInnerGradSimple)
