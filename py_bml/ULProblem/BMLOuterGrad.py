...
from py_bml.LLProblem import BMLInnerGradTrad, BMLInnerGradAggr, BMLInnerGradSimple

RAISE_ERROR_ON_DETACHED = False


class BMLOuterGrad(object):
    ...
    def compute_gradients(self, outer_objective, bml_inner_grad, meta_param=None):
        ...
        """
        assert isinstance(bml_inner_grad, (BMLInnerGradAggr, BMLInnerGradSimple, BMLInnerGradTrad)), \
            BMLOuterGrad._ERROR_NOT_OPTIMIZER_DICT.format(bml_inner_grad)
        self._optimizer_dicts.add(bml_inner_grad)

        if meta_param is None:  # get default outer parameters
            meta_param = py_bml.extension.meta_parameters(tf.get_variable_scope().name)
        return meta_param
    ...
    def hgrads_hvars(self, meta_param=None, aggregation_fn=None, process_fn=None):
        ...
        def _aggregate_process_manage_collection(_hg_lst):
            ...
            tf.add_to_collection(py_bml.extension.GraphKeys.OUTERGRADIENTS, aggr)
            return aggr
