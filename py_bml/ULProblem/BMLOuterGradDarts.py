...
class BMLOuterGradDarts(BMLOuterGrad):
    ...
    def __init__(self, inner_method='Reverse', truncate_iter=-1, name='BMLOuterGradDarts'):
       ...
       :return: ReverseHG object
           """
        super(BMLOuterGradDarts, self).__init__(name)
    ...
    def compute_gradients(self, outer_objective, optimizer_dict, meta_param=None, param_dict=OrderedDict()):
        ...
        """
        meta_param = super(BMLOuterGradDarts, self).compute_gradients(outer_objective, optimizer_dict, meta_param)
    ...
    def _create_hyper_gradients(hyper_grads, meta_param):
        ...
        [tf.add_to_collection(py_bml.extension.GraphKeys.OUTERGRADIENTS, hyper_grad) for hyper_grad in hyper_gradients]
    ...
    def _create_darts_derivatives(var_list, darts_derivatives):
        ...
        [tf.add_to_collection(py_bml.extension.GraphKeys.DARTS_DERIVATIVES, der) for der in derivatives]
        py_bml.extension.remove_from_collection(py_bml.extension.GraphKeys.GLOBAL_VARIABLES, *derivatives)
        # this prevents the 'automatic' initialization with tf.global_variables_initializer.
        return derivatives
    ...
    def _create_outergradient_from_dodh(hyper, doo_dhypers):
        ...
        py_bml.extension.remove_from_collection(py_bml.extension.GraphKeys.GLOBAL_VARIABLES, hgs)
        return hgs

    @staticmethod
    def _create_outergradient(outer_obj, hyper):
        return BMLOuterGradDarts._create_outergradient_from_dodh(hyper, tf.gradients(outer_obj, hyper)[0])
