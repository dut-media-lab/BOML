...
class BMLOuterGradReverse(BMLOuterGrad):
    ...
    def __init__(self, inner_method='Reverse', truncate_iter=-1, name='BMLOuterGradReverse'):
       ...
       :return: ReverseHG object
           """
        super(BMLOuterGradReverse, self).__init__(name)
    ...
    def compute_gradients(self, outer_objective, optimizer_dict, meta_param=None, param_dict=OrderedDict()):
        ...
        """
        meta_param = super(BMLOuterGradReverse, self).compute_gradients(outer_objective, optimizer_dict, meta_param)
        ...
        with tf.variable_scope(outer_objective.op.name):
            ...
            for dl_dh, a_d_b0, hyper in zip(alpha_dot_B, alpha_dot_B0, meta_param):
                assert dl_dh is not None or a_d_b0 is not None, BMLOuterGrad._ERROR_HYPER_DETACHED.format(hyper)
                hgv = None
    ...
    def _create_lagrangian_multipliers(optimizer_dict, doo_ds):
        ...
        [tf.add_to_collection(py_bml.extension.GraphKeys.LAGRANGIAN_MULTIPLIERS, lm) for lm in lag_mul]
        py_bml.extension.remove_from_collection(py_bml.extension.GraphKeys.GLOBAL_VARIABLES, *lag_mul)
    ...
    def _create_outergradient(outer_obj, hyper):
        return BMLOuterGradReverse._create_outergradient_from_dodh(hyper, tf.gradients(outer_obj, hyper)[0])
