...
from py_bml import utils, extension
from py_bml.DataLoader import ImageNetMetaDataset, OmniglotMetaDataset
from py_bml.DataLoader.experiment import BMLExperiment

importlib = __import__('importlib')
bml_networks = importlib.import_module('py_bml.Networks')
inner_grads = importlib.import_module('py_bml.LLProblem')
hyper_grads = importlib.import_module('py_bml.ULProblem')
bml_optimizer = importlib.import_module('py_bml.Optimizer')


class BMLHOptimizer(object):
    """
    ...
    def __init__(self, Method=None, inner_method=None, outer_method=None, truncate_iter=-1,experiments=[]):
        ...
        :param Method: define basic method for following training process, it should be included in ['HyperOptim', 'BilevelOptim'],
         'HyperOptim' type includes methods like 'MAML, FOMAML, TNet, WarpGrad'; 'BilevelOptim' type includes methods like
         'BDA, RHG, TRHG, Implicit HG, DARTS';
        :param inner_method: method chosen for solving LLproblem, including ['Trad' ,'Simple', 'Aggr'], 'BilevelOptim' type choose
        either 'Trad' for traditional optimization strategies or 'Aggr' for Gradient Aggragation optimization 'HyperOptim' type
        ...
        'HyperOptim' type should choose Simple, and set specific parameters for detailed method choices like FOMAML
        ...
        assert Method in (
            'BilevelOptim',
            'HyperOptim'), 'initialize method arguement, should be in list \an [BilevelOptim,HyperOptiml]' \
                           'BilevelOptim based methods include [BDA,FHG,RHG,TRHG],' \
        ...
        self._inner_gradient = getattr(inner_grads, '%s%s' % ('BMLInnerGrad', self.inner_method))
        if truncate_iter >= 0:
            self._outer_gradient = getattr(hyper_grads, '%s%s' % ('BMLOuterGrad', self.outer_method))(
    ...
    def Meta_model(self, _input, dataset, meta_model='v1', name='Hyper_Net', use_T=False, use_Warp=False,**model_args):
        """
        ...
        if use_Warp:
            if 'model_loss_func' in model_args.keys():
                ...
                self.param_dict['model_loss_func'] = utils.cross_entropy_loss
            if 'outer_loss_func' in model_args.keys():
                self.param_dict['outer_loss_func'] = model_args['outer_loss_func']
            else:
                self.param_dict['outer_loss_func'] = utils.cross_entropy_loss
        ...
        assert meta_model in ('v1', 'v2'), \
            'The dataset does not match the model chosen for meta_learner'
        if self.Method == 'HyperOptim':
            if isinstance(dataset.train, OmniglotMetaDataset):
                meta_learner = getattr(bml_networks, '%s' % ('BMLNetOmniglotHO_' + meta_model))(
                ...
                meta_learner = getattr(bml_networks, '%s' % ('BMLNetMiniHO_' + meta_model))(
                    _input=_input, dim_output=dataset.train.dim_target, name=name, use_T=use_T, use_Warp=use_Warp,**model_args)
        elif self.Method == 'BilevelOptim':
            if isinstance(dataset.train, OmniglotMetaDataset):
                meta_learner = getattr(bml_networks, '%s' % ('BMLNetOmniglotMetaRepr_' + meta_model))(
                    _input=_input, name=name, use_T=use_T, outer_method=self.outer_method, **model_args)
            elif isinstance(dataset.train, ImageNetMetaDataset):
                meta_learner = getattr(bml_networks, '%s' % ('BMLNetMiniMetaRepr_' + meta_model))(
                    _input=_input, name=name, use_T=use_T, outer_method=self.outer_method,**model_args)
        else:
            print('initialize method arguement, should be in list \an [BilevelOptim,HyperOptiml]')
            raise AssertionError
    ...
    def Base_model(self, _input, meta_learner, name='Task_Net',
        ...
        :param weights_initializer: initializer function for task_specific network, called by 'BilevelOptim' method
        :return: task-specific model part
        '''
        if self.Method == 'HyperOptim':
            if isinstance(self.data_set.train, OmniglotMetaDataset):
                base_learner = getattr(bml_networks, '%s' % ('BMLNetOmniglotHO_' + self.param_dict['meta_model']))(
                ...
                base_learner = getattr(bml_networks, '%s' % ('BMLNetMiniHO_' + self.param_dict['meta_model']))(
                    _input=_input, outer_param_dict=meta_learner.outer_param_dict, model_param_dict=meta_learner.model_param_dict,
                    dim_output=meta_learner.dims[-1], name=name, use_T=meta_learner.use_T,use_Warp=meta_learner.use_Warp)
        elif self.Method == 'BilevelOptim':
            base_learner = getattr(bml_networks, 'BMLNetFeedForward')(
    ...
    def LL_problem(self, inner_objective, learning_rate, T, inner_objective_optimizer='SGD', outer_objective=None,
                   ...
                   init_dynamics_dict=None, first_order=False, loss_func=utils.cross_entropy_loss, momentum=0.5,beta1=0.0,beta2=0.999,
        ...
        assert isinstance(experiment,
                          BMLExperiment), 'HyperOptim based methods require specialized ' \
    ...
    def UL_problem(self, outer_objective, meta_learning_rate, inner_grad,
        ...
        if self.oo_opt is None:
            if outer_objective_optimizer == 'Momentum':
                ...
                raise IndexError
        assert isinstance(self._outer_gradient, getattr(hyper_grads, 'BMLOuterGrad')), \
        ...
        if 'Reptile' not in self.param_dict.keys() and reptile:
            assert isinstance(self._outer_gradient, getattr(hyper_grads, 'BMLOuterGradSimple')), \
        ...
        if self.outer_method == 'Darts' and (not hasattr(self.outergradient, 'Epsilon')):
            ...
            assert isinstance(self._outer_gradient, getattr(hyper_grads, 'BMLOuterGradReverse')), \
                'Wrong name for outer method,should be in list [Simple]'
    ...
    def Aggregate_all(self, aggregation_fn=None, gradient_clip=None):
        ...
        if self._fin_hts is None:
            ...
            raise ValueError('HyperOptimizer.Aggregate_all has already been called on ' +
