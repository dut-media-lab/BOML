...
from boml.ul_problem import BOMLOuterGrad
...
class BOMLOuterGradImplicit(BOMLOuterGrad):
    ...
    def compute_gradients(self, outer_objective, optimizer_dict, meta_param=None, param_dict=OrderedDict()):
        meta_param = super(BOMLOuterGradImplicit, self).compute_gradients(outer_objective, optimizer_dict, meta_param)
        ...
        with tf.variable_scope(outer_objective.op.name):
            ...
            for g2, cd, hyper in zip(g2s, cross_ders, meta_param):
                assert g2 is not None or cd is not None, BOMLOuterGrad._ERROR_HYPER_DETACHED.format(hyper)
