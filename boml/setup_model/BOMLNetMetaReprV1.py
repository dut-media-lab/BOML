    ...
    def _forward(self):
        ...
        for i in range(len(self.dim_hidden)):
            if self.use_T:
                ...
                self + network_utils.conv_block_Warp(self, self.outer_param_dict['conv' + str(i)], self.outer_param_dict['bias' + str(i)],
                                         self.model_param_dict['conv' + str(i) + '_z'], self.model_param_dict['bias'+str(i)+'_z'])
            else:
                self + network_utils.conv_block(self,  zzzzzself.outer_param_dict['conv' + str(i)], self.outer_param_dict['bias' + str(i)])
        if self.flatten:
            flattened_shape = reduce(lambda a, v: a * v, self.layers[-1].get_shape().as_list()[1:])
            self + tf.reshape(self.out, shape=(-1, flattened_shape), name='representation')
        else:
