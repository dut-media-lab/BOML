...
class BOMLOuterGradForward(BOMLOuterGrad):
    ...
    def compute_gradients(self, outer_objective, optimizer_dict, meta_param=None, param_dict=OrderedDict()):
        meta_param = super(BOMLOuterGradForward, self).compute_gradients(outer_objective, optimizer_dict, meta_param)
        ...
        with tf.variable_scope(outer_objective.op.name):
            ...
            for hyp in meta_param:
                assert hyp.shape.ndims == 0, BOMLOuterGradForward._HYPER_RANK_ERROR_MESSAGE.format(hyp, hyp.shape.ndims)
                ...
                if RAISE_ERROR_ON_DETACHED:
                    # try:
                    assert hyper_ok, BOMLOuterGrad._ERROR_HYPER_DETACHED.format(hyp)
                    # ex
                else:
                    if not hyper_ok:
                        print(BOMLOuterGrad._ERROR_HYPER_DETACHED.format(hyp), file=sys.stderr)
