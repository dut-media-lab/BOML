...
class BOMLOptMomentum(BOMLOpt, tf.train.MomentumOptimizer):
    def __init__(self, learning_rate, momentum, use_locking=False, name="Momentum",
        ...
        super(BOMLOptMomentum, self).__init__(learning_rate, momentum, use_locking, name, use_nesterov)

    def apply_gradients(self, grads_and_vars, global_step=None, name=None):
        # filter_hypers

        update_op = super(BOMLOptMomentum, self).apply_gradients(grads_and_vars, global_step, name)
    ...
    def optimizer_params_tensor(self):
        return super(BOMLOptMomentum, self).optimizer_params_tensor + [self._momentum_tensor]
